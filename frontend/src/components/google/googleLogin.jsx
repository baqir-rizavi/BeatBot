...
function GoogleButton() {
    ...
    const responseGoogle = (response) => {
        const requestOptions = {
            ...
            body: JSON.stringify({ "name": response.profileObj.name, "email": response.profileObj.email, "pic_link": response.profileObj.imageUrl})
        };
        sendGoogleInfo(requestOptions);
    };
      
    const sendGoogleInfo = async (req) => {
          fetch("/login_with_google", req).then(
            result => result.json()
          ).then(
              data => {
                if (data.code == "ok"){
             ...
             <GoogleLogin 
                ...
                render={renderProps => (
                    <button onClick={renderProps.onClick} type="button">
                        ...
                        <span className='bhal'>login with google</span>
                    </button>
